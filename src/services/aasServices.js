...
export default class aasServices {
    ...
    getAasShellId(assetIds, requestHeaders) {
        return new Promise((resolve) => {
            ...
            axios.get(`${BASE_URL}/registry/lookup/shells?assetIds=${encodedAssetIds}`,
                    {
    ...
    getShellDescriptor(aasShellId, requestHeaders) {
        return new Promise((resolve) => {
            axios
                .get(`${BASE_URL}/registry/registry/shell-descriptors/${aasShellId}`,
                    {
                        headers: requestHeaders
                    }
                )
    ...
    getSubmodelDescriptor(shellDescriptor, requestHeaders) {
        return new Promise((resolve) => {
            axios
                .get(
                    `${BASE_URL}/registry/registry/shell-descriptors/${shellDescriptor.identification}/submodel-descriptors/${shellDescriptor.submodelDescriptors[0].identification}`,
                    {
                        headers: requestHeaders
                    }
                )
